...
const QuestTwo = ({ onSuccess }) => {
  ...
  const correctCode='CYF33SSN99';
  const { coins, unlockWithCoins }=useCoinUnlock();
  const mapRef=useRef(null);
  
  const secretPlace={lat:50.912345,lng:34.822345};
  const secretRadius= 3;
  const proximityRadius= 300;

  const tabs=[
    {title:'Live Camera & Mystical Map',mapUrl:Maps[1].map1},
    {title:'Secret location description, clues, etc.',text:{one:'River bank. Look for the bridge.',two:'Fountain. Landmark near the secret place.'}}
  ];

  useProximityToast({...secretPlace,radius:proximityRadius},"You’ve entered the secret zone. Secret Place 2 is nearby!");
  ...
  const handleSubmit=async e=>{
    ...
    try{
      ...
      if(snap.data().questTwoCompleted) return toast.info('Quest already completed.');
      await updateDoc(userRef,{cyfCoins:increment(1),questTwoCompleted:true});
      toast.success('Secret Place 2 unlocked! +1 CYF Coin');
      onSuccess();
      ...
      <div className="quests__container container">
        ...
        <div className="form-question">
          <form onSubmit={handleSubmit} className="form form-quest position-relative">
            <input type="text" value={codeInput} onChange={e=>setCodeInput(e.target.value)} placeholder="Write secret code for Secret Place 2" className="form__input input__field-two"/>
            <div className='coins-item-unlock'>
              <button type="submit" className="form__button button enter-btn">Enter Code</button>
              <button className="button form__button unlock-btn" onClick={e=>{e.preventDefault(); unlockWithCoins(3, async ()=>{ try{ const user=auth.currentUser;if(!user) return; const ref=doc(db,'users',user.uid); await updateDoc(ref,{questTwoCompleted:true}); onSuccess(); toast.success('Secret Place 2 unlocked via coins!'); }catch(err){console.error(err);toast.error('Error unlocking quest via coins'); } }); }}>
            <p className='coins-counter'>
              Your coins: <span>{coins}</span></p>
              Unlock Quest</button>
            </div>
          </form>
        </div>
