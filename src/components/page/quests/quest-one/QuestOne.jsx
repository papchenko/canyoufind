...
const QuestOne = ({ onSuccess }) => {
  ...
  const tabs = [
    { title: 'Live Camera & Mystical Map', mapUrl: Maps[0].map1 },
    { title: 'Secret location description, clues, etc.', text: { one:'A place of rest, a beach.', two:'There is a tennis court nearby.' } }
  ];

  useProximityToast({ ...secretPlace, radius: proximityRadius }, "You’ve entered the secret zone. Secret Place 1 is nearby!");

  const getDistance=(lat1,lon1,lat2,lon2)=>{
    const R=6371000,dLat=(lat2-lat1)*Math.PI/180,dLon=(lon2-lon1)*Math.PI/180,a=Math.sin(dLat/2)**2+Math.cos(lat1*Math.PI/180)*Math.cos(lat2*Math.PI/180)*Math.sin(dLon/2)**2,c=2*Math.atan2(Math.sqrt(a),Math.sqrt(1-a));
    return R*c;
  };
  ...
  useEffect(()=>{
    ...
    const watchId=navigator.geolocation.watchPosition(
      pos=>{
        ...
        setSecretVisible(getDistance(posObj.lat,posObj.lng,secretPlace.lat,secretPlace.lng)<=arRadius);
      },
  ...
  const handleSubmit=async e=>{
    ...
    try{
      ...
      if(snap.data().questOneCompleted) return toast.info('Quest already completed.');
      await updateDoc(userRef,{cyfCoins:increment(1),questOneCompleted:true});
      toast.success('Secret Place 1 unlocked! +1 CYF Coin');
      onSuccess();
      ...
      <div className="quests__container container">
        ...
        <div className="row">
          ...
          <div className="col-lg-6 explore-content text-white">
            ...
            {activeIndex===0 && (
              <MapContainer center={[secretPlace.lat,secretPlace.lng]} zoom={14} style={{height:'400px',width:'100%',borderRadius:'12px',marginBottom:'20px'}} whenCreated={map=>{mapRef.current=map}} zoomControl={false}>
        ...
        <div className="form-question">
          <form onSubmit={handleSubmit} className="form form-quest position-relative">
            <input type="text" value={codeInput} onChange={e=>setCodeInput(e.target.value)} placeholder="Write secret code for Secret Place 1" className="form__input input__field-two"/>
            <div className='coins-item-unlock'>
              <button type="submit" className="form__button button enter-btn">Enter Code</button>
              <button className="button form__button unlock-btn" onClick={e=>{e.preventDefault(); unlockWithCoins(3, async ()=>{
                try{ const user=auth.currentUser; if(!user) return; const ref=doc(db,'users',user.uid); await updateDoc(ref,{questOneCompleted:true}); onSuccess(); toast.success('Secret Place 1 unlocked via coins!'); } catch(err){console.error(err); toast.error('Error unlocking quest via coins'); }
              }); }}>
